...
class MyAlgorithm(threading.Thread):
    ...
    def algorithm(self):
        ...
        if self.ukf == None:
            ...
            self.ukf.Q  = np.diag([(0.225**2),(0.225**2)]) #imu variance #0.225**2
            self.ukf.R = np.diag([(0.2**2),(0.2**2)]) #vo variance #0.1**2
        ...
        pos_current = self.ukf.x

        V_avg_x = ((self.ax_prev + self.ax_current) *self.d_t)/2
        V_avg_y = ((self.ay_prev + self.ay_current)*self.d_t)/2

        t_imu = np.zeros((3,1) , dtype = float)
        t_imu[0,0] = V_avg_x * self.d_t
        t_imu[1,0] = V_avg_y * self.d_t
        ...
        R , t ,scale , mask = self.vo.get_rot_trans(color_image , depth_image)
        pos_t = np.array([[pos_current[0]],
                            [0],
                            [pos_current[1]]] , dtype = float).reshape((3,1))
        pos_r = np.dot(R,self.R_prev)
        #print("global scale" ,abs(t_imu[0,0]/t[2]) ,abs(t_imu[1,0]/t[0]) )
        #global_scale  = (abs(t_imu[0,0]/t[2]) + abs(t_imu[1,0]/t[0]))/2
        ...
        pos_t = pos_t + np.dot(pos_r , t) * scale / 35
        z= np.zeros((2) , dtype = float)
        #z  = np.array((-pos_t[2] , -pos_t[0]) ,dtype = float )
        z[0] = -pos_t[2]
        z[1] = -pos_t[0]
        print("vo" , z)
        self.ukf.update(z)
        ...
        self.set_predicted_pose(imu_x  ,imu_y)
